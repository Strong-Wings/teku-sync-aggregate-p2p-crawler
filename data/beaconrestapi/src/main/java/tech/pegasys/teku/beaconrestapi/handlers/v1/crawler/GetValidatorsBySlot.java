...
public class GetValidatorsBySlot extends MigratingEndpointAdapter {
  ...
  public GetValidatorsBySlot(final DataProvider provider) {
    this(provider.getNetworkDataProvider(), provider.getChainDataProvider());
  }

  GetValidatorsBySlot(final NetworkDataProvider network, final ChainDataProvider chainDataProvider) {
    super(
        EndpointMetadata.get(ROUTE)
            .operationId("getValidatorsBySlot")
  ...
  public void handleRequest(RestApiRequest request) throws JsonProcessingException {
    ...
    var allValidators = chainDataProvider.getStateSyncCommittees(request.getPathParameter(SLOT_PARAMETER).toString(), Optional.empty())
            .join()
            .map(ObjectAndMetaData::getData)
            .map(StateSyncCommitteesData::getValidators)
